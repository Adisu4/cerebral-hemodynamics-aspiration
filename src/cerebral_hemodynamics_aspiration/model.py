...
def jugular_conductances(y: np.ndarray, p: ModelParameters) -> dict[str, float]:
...
def cerebral_arterioles_resistance(
    Cpa: float, Ppa: float, Pic: float, p: ModelParameters
) -> float:
...
def cerebral_veins_resistance(
    Pv: float, Pic: float, Pvs: float, p: ModelParameters
) -> tuple[float, str]:
...
def cerebral_veins_capacitance(
    Pv: float, Pic: float, p: ModelParameters
) -> float:
...
def superior_vena_cava_junction(
    y: np.ndarray, conductances: Mapping[str, float], p: ModelParameters
) -> float:
