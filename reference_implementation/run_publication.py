"""Converged, provenance-rich simulation runner for the publication model."""
...
DEFAULT_OUT = ROOT / "results" / "publication"
...
def source_hashes() -> dict[str, str]:
    files = (
        ROOT / "publication_model.py",
        ROOT / "publication_parameters.py",
        ROOT / "run_publication.py",
        ROOT / "run_publication_study.py",
        ROOT / "test_publication_model.py",
    )
    return {
...
def integrate(
    ...
    *,
    ...
    if not overwrite and json_path.exists() and npz_path.exists():
...
def load_report(label: str, output_dir: Path | str = DEFAULT_OUT) -> dict:
    return json.loads((Path(output_dir) / f"{label}.json").read_text(encoding="utf-8"))


__all__ = ["DEFAULT_OUT", "IntegratorConfig", "integrate", "load_report", "source_hashes"]
