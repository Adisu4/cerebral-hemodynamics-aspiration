"""Run the complete source-audited publication study.
...
from publication_parameters import make_reference_parameters, make_tbi_parameters
from run_publication import DEFAULT_OUT, IntegratorConfig, integrate, load_report, source_hashes
...
def maybe_run(force: bool, output_dir: Path, *args, **kwargs) -> dict:
    label = kwargs["label"]
    if not force and (output_dir / f"{label}.json").exists() and (output_dir / f"{label}.npz").exists():
        report = load_report(label, output_dir)
        print(f"{label}: reused", flush=True)
        return report
    return integrate(*args, output_dir=output_dir, overwrite=True, **kwargs)
...
def main(force: bool = False, output_dir: Path = DEFAULT_OUT) -> None:
